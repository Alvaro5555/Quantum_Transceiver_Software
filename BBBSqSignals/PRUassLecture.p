...
REGISTERCNT:
	SBCO 	r7, CONST_PRUDRAM, 12, 4
	SBCO 	r3, CONST_PRUSHAREDRAM, r1, 4
	ADD 	r1, r1, 4

WAIT_FOR_EVENT:
	// Detección de flanco de subida, optimizada para velocidad
	MOV 	r16.w0, r31.w0
	NOT		r16, r16
	MOV		r6.w0, r31.w0
	QBEQ 	WAIT_FOR_EVENT, r6.w0, 0 // Salida rápida si no hay ningún pin en alto

	// --- NUEVO: Captura del timestamp antes de la ramificación ---
	// Si un pin está en alto, capturamos el tiempo INMEDIATAMENTE para máxima precisión
	LBBO	r5, r13, 0, 4 // Lee el valor de DWT_CYCCNT

	// Ahora, validamos que sea un flanco de subida
	AND		r6, r6, r16 // Aísla solo los bits que han pasado de 0 a 1

	// --- NUEVO: Ramificación por canal ---
	// Comprueba cada bit (canal) y salta a su manejador específico.
	// Si ocurren múltiples eventos en el mismo ciclo, se procesará el de menor número.
	QBBS	TIMETAG_CH1, r6, 0 // Si el bit 0 (Canal 1) está activo, salta
	QBBS	TIMETAG_CH2, r6, 1 // Si el bit 1 (Canal 2) está activo, salta
	QBBS	TIMETAG_CH3, r6, 2 // Si el bit 2 (Canal 3) está activo, salta
	QBBS	TIMETAG_CH4, r6, 3 // Si el bit 3 (Canal 4) está activo, salta

	// Si no fue un flanco de subida válido en ningún canal (muy raro), vuelve a esperar
	JMP 	WAIT_FOR_EVENT

// --- NUEVO: Bloques de manejo para cada canal ---
// Ahora tienes secciones separadas, igual que en tu script transmisor.
// Por ahora, todas hacen lo mismo, pero puedes personalizarlas fácilmente.

TIMETAG_CH1:
	// Acción específica para el Canal 1.
	// El registro r6 contendrá solo el bit 0 activo (valor 0x01).
	SBCO 	r5, CONST_PRUSHAREDRAM, r1, 5 // Guarda 4 bytes de r5 y 1 byte de r6
	ADD 	r1, r1, 5
	JMP		EVENT_HANDLED

TIMETAG_CH2:
	// Acción específica para el Canal 2.
	// El registro r6 contendrá solo el bit 1 activo (valor 0x02).
	SBCO 	r5, CONST_PRUSHAREDRAM, r1, 5
	ADD 	r1, r1, 5
	JMP		EVENT_HANDLED

TIMETAG_CH3:
	// Acción específica para el Canal 3.
	// El registro r6 contendrá solo el bit 2 activo (valor 0x04).
	SBCO 	r5, CONST_PRUSHAREDRAM, r1, 5
	ADD 	r1, r1, 5
	JMP		EVENT_HANDLED

TIMETAG_CH4:
	// Acción específica para el Canal 4.
	// El registro r6 contendrá solo el bit 3 activo (valor 0x08).
	SBCO 	r5, CONST_PRUSHAREDRAM, r1, 5
	ADD 	r1, r1, 5
	JMP		EVENT_HANDLED

// --- NUEVO: Bloque común para finalizar el manejo del evento ---
EVENT_HANDLED:
