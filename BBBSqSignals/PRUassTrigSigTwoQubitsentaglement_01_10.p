...
SIGNALON4STATE1:	// The odd signals actually carry the signal (so it is half of the period, adjusting the on time); while the even signals are the half period alway off
	MOV		r30.w6, r11.w6 // Double channels 2. write to magic r30 output byte 0. Half word bytes= 7,6,5,4,3,2,1,0 bits
	MOV		r5, r14
SIGNALON4DELSTATE1:
	SUB		r5, r5, 1
	QBNE	SIGNALON4DELSTATE1, r5, 0
//	LDI		r4, 0 // Controlled intentional delay to account for the fact that QBNE takes one extra count when it does not go through the barrier
SIGNALOFF4STATE1: // Make use of this dead time to instantly correct for intra relative frequency sifference
	MOV		r30.w6, 0x0000 // All off
	MOV		r5, r15
//	LDI		r4, 0 // Intentionally controlled delay to adjust all sequences (in particular to the last one)
SIGNALOFF4DELSTATE1:
	SUB		r5, r5, 1
	QBNE	SIGNALOFF4DELSTATE1, r5, 0
STATE1QUBITCOUNT:
	SUB		r20, r20, 1
...
SIGNALON3STATE2:	// The odd signals actually carry the signal (so it is half of the period, adjusting the on time); while the even signals are the half period alway off
	MOV		r30.w4, r11.w4 // Double channels 2. write to magic r30 output byte 0. Half word bytes= 7,6,5,4,3,2,1,0 bits
	MOV		r5, r14
SIGNALON3DELSTATE2:
	SUB		r5, r5, 1
	QBNE	SIGNALON3DELSTATE2, r5, 0
//	LDI		r4, 0 // Controlled intentional delay to account for the fact that QBNE takes one extra count when it does not go through the barrier
SIGNALOFF3STATE2: // Make use of this dead time to instantly correct for intra relative frequency sifference
	MOV		r30.w4, 0x0000 // All off
	MOV		r5, r15
//	LDI		r4, 0 // Intentionally controlled delay to adjust all sequences (in particular to the last one)
SIGNALOFF3DELSTATE2:
	SUB		r5, r5, 1
	QBNE	SIGNALOFF3DELSTATE2, r5, 0
STATE2QUBITCOUNT:
	SUB		r20, r20, 1
...
STATE4QUBIT: //Qubit1 1 Qubit2 0 base 2
	MOV		r20, PULSE_COUNT
SIGNALON3STATE4:	// The odd signals actually carry the signal (so it is half of the period, adjusting the on time); while the even signals are the half period alway off
	MOV		r30.w4, r11.w4 // Double channels 2. write to magic r30 output byte 0. Half word bytes= 7,6,5,4,3,2,1,0 bits
	MOV		r5, r14
SIGNALON3DELSTATE4:
	SUB		r5, r5, 1
	QBNE	SIGNALON3DELSTATE4, r5, 0
//	LDI		r4, 0 // Controlled intentional delay to account for the fact that QBNE takes one extra count when it does not go through the barrier
...
SIGNALOFF3DELSTATE4:
	SUB		r5, r5, 1
	QBNE	SIGNALOFF3DELSTATE4, r5, 0
SIGNALON4STATE4:	// The odd signals actually carry the signal (so it is half of the period, adjusting the on time); while the even signals are the half period alway off
	MOV		r30.w6, r11.w6 // Double channels 2. write to magic r30 output byte 0. Half word bytes= 7,6,5,4,3,2,1,0 bits
	MOV		r5, r14
SIGNALON4DELSTATE4:
	SUB		r5, r5, 1
	QBNE	SIGNALON4DELSTATE4, r5, 0
//	LDI		r4, 0 // Controlled intentional delay to account for the fact that QBNE takes one extra count when it does not go through the barrier
SIGNALOFF4STATE4: // Make use of this dead time to instantly correct for intra relative frequency sifference
	MOV		r30.w6, 0x0000 // All off
	MOV		r5, r15
//	LDI		r4, 0 // Intentionally controlled delay to adjust all sequences (in particular to the last one)
SIGNALOFF4DELSTATE4:
	SUB		r5, r5, 1
	QBNE	SIGNALOFF4DELSTATE4, r5, 0
STATE4QUBITCOUNT:
	SUB		r20, r20, 1
	QBNE	SIGNALON3STATE4, r20, 0
